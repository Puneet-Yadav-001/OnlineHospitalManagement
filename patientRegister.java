...
public class patientRegister {
    JFrame frame;
    JLabel heading, logo, signup, name, email, father, phone, password, gender, dieases, dob, state;
    JTextField uname, emailadd, phoneno, fname;
    JPasswordField pass;
    JRadioButton male, female;
    JButton show, register, cancel, reset, back;
    ButtonGroup b1;
    JComboBox statetf, dieasestype;
    ...
    String[] diease = {"Select dieases type", "Colon and Rectal Surgeons", "Hematologists", "Internists", "Medical Geneticists",
    ...
    patientRegister(){
        ...
        dieases = new JLabel("Diseases");
        dieases.setBounds(150, 550, 200, 40);
        dieases.setFont(new Font("Arial", Font.PLAIN, 25));
        frame.add(dieases);
        ...
        dieasestype = new JComboBox(diease);
        dieasestype.setBounds(350, 550, 270, 30);
        dieasestype.setFont(new Font("Arial", Font.PLAIN, 20));
        dieasestype.setBorder(BorderFactory.createLineBorder(Color.BLACK, 2));
        frame.add(dieasestype);
        ...
        register.addActionListener(new ActionListener() {
            ...
            public void actionPerformed(ActionEvent e) {
                ...
                if (uname.getText().isEmpty()){
                    ...
                    return;
                } else if (dieasestype.getSelectedItem().equals("Select dieases type")) {
                ...
                arr[8] = (String) dieasestype.getSelectedItem();

                try{
                    conn c = new conn();
                    String query1 = "select patientid from patienthistory where username = '" + arr[0] + "' and fathername = '" + arr[1] + "' and gender = '" + arr[2] + "' and email = '" + arr[3] + "' and phone = " + arr[4] + " and password = '" + arr[5] + "' and dob = '" + arr[6] + "' and state = '" + arr[7] + "' and dieasestype = '" + arr[8] + "' and active = 'yes'";
                    ResultSet rs = c.s.executeQuery(query1);
                    while(rs.next()){
                        JOptionPane.showMessageDialog(null, "Patient with same diease already exist.");
                        return;
                    }
                    String query2 = "INSERT INTO `patienthistory`(`username`, `fathername`, `gender`, `email`, `phone`, `password`, `dob`, `state`, `dieasestype`, `active`, `joinDate`) VALUES ('" + arr[0] + "','" + arr[1] + "','" + arr[2] + "','" + arr[3] + "'," + arr[4] + ",'" + arr[5] + "','" + arr[6] + "','" + arr[7] + "','" + arr[8] + "', 'yes', '" + java.time.LocalDate.now() + "')";
                    c.s.execute(query2);
                    String query3 = "select patientid from patienthistory where username = '" + arr[0] + "' and fathername = '" + arr[1] + "' and gender = '" + arr[2] + "' and email = '" + arr[3] + "' and phone = " + arr[4] + " and password = '" + arr[5] + "' and dob = '" + arr[6] + "' and state = '" + arr[7] + "' and dieasestype = '" + arr[8] + "' and active = 'yes'";
                    ResultSet rs1 = c.s.executeQuery(query3);
                    while(rs1.next()){
                        JOptionPane.showMessageDialog(null, "Patient id is " + rs1.getString(1));
                        String query4 = "INSERT INTO `login`(`userid`, `username`, `password`, `type`, `active`) VALUES ('" + rs1.getString(1) + "','" + arr[0] + "','" + arr[5] + "','Patient', 'yes')";
                        c.s.execute(query4);
                        break;
                    }
                } catch (Exception ae){
        ...
        reset.addActionListener(new ActionListener() {
            ...
            public void actionPerformed(ActionEvent e) {
                ...
                emailadd.setText("");
                dieasestype.setSelectedIndex(0);
                phoneno.setText("");
                pass.setText("");
        ...
        dieasestype.addKeyListener(new KeyAdapter() {
            @Override
            public void keyPressed(KeyEvent e) {
                if (e.getKeyCode() == KeyEvent.VK_ENTER) {
                    register.doClick();
